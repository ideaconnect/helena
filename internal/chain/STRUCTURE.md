...
| `resolveSteps` | Walks one request's chain. For each step: validates the alias (a JS identifier that is neither a reserved word nor a JS Object built-in property name — `jsObjectProps`, so `__proto__`/`constructor`/… can't corrupt the `chain` object) and the reference (present when either `Request` **or** `RequestID` is set — an ID-only step is valid), looks up the predecessor via `resolveTarget` (ID-first, path-fallback), recursively resolves its own chain, fires `progress` (if any), executes the step via the supplied executor, and accumulates the result. Cycle detection is keyed by `Request.ID` in the shared `visiting` map. `stepRef` renders a step's identifier (path, else `id <RequestID>`) for error messages. |
