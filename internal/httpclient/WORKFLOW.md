...
1a. **Path-parameter fill.** Immediately after resolving `r.URL`, `Build` runs [`pathparam.Apply`](../pathparam/) over the resolved URL to fill single-brace `{name}` placeholders from `r.PathParams`. Each enabled entry's value is run through the same `resolve` closure (so a path value may itself reference `{{vars}}`/`{{?prompts}}`, reported into `missing`); an unfilled or empty token is left literal as `{name}`. `{{...}}` spans are already resolved by this point, and `Apply` never re-scans inserted values, so a value containing braces cannot trigger further substitution.
...
10. **Dispatch.** `(*Client).Do` records `start := time.Now()`, calls `c.http.Do(req)`, and defers `Body.Close()`. If a cookie jar was installed (`SetCookieJar`, #91), `net/http` adds matching jar cookies to `req` here (appended to any user-set `Cookie` header) and stores the response's `Set-Cookie` back into the jar — automatically across redirect hops. Because `AddCookie` mutates the request, `req.Header.Get("Cookie")` reflects the jar cookies afterward, so the credentialed-CORS check in step 12 sees them. (Caveat: that check is a best-effort heuristic for the *origin* request — on a cross-host redirect chain net/http builds a fresh request per hop and the final hop's jar cookies land there, not on the original `req`, so the advisory reflects the first hop's credential state, not the redirect target's. The advisory is non-enforcing, so this is acceptable.) A transport error is passed through `sanitizeDoError` first: Go's `*url.Error` carries the resolved request URL verbatim, which may include userinfo (`user:pass@host`) or a `{{token}}`-interpolated query, so `redactURL` masks the userinfo and the query string (host + path stay visible) before the error reaches the UI status line (#112).
