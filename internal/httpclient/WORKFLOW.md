...
10. **Dispatch.** `(*Client).Do` records `start := time.Now()`, calls `c.http.Do(req)`, and defers `Body.Close()`.
11. **Response capture.** `io.ReadAll` drains the body into memory; `Duration` is `time.Since(start)`. Failures during read return `"reading response body: %w"`.
