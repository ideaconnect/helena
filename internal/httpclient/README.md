...
- `(*Client).Stream(ctx, r, res, onOpen, onEvent) error` — Server-Sent Events (#74): sends with `Accept: text/event-stream`, reads the body incrementally (not capped), and delivers parsed [sse.Event](../sse/)s to `onEvent` until the stream ends, `onEvent` returns false, or `ctx` is cancelled. A non-2xx status is an error. `StreamMeta` carries the open-time status/headers. `TimeoutSeconds` bounds only the connect + response-header phase — an open stream is never killed by the client-wide timeout (that deadline covers the body read, which for SSE is the stream itself).
