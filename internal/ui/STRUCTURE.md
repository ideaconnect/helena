...
| [auth.go](auth.go) | `buildAuthTab`, `loadAuthTab`, `refreshAuthVisibility`, `refreshAuthInheritLabel`, and the `ensureBasic`/`ensureBearer`/`ensureWSSE` (#79)/`ensureAPIKey`/`ensureOAuth2` lazy allocators for the per-type sub-structs. |
