...
| [send.go](send.go) | The Send lifecycle: `send` (off-UI goroutine → chain.Resolve → ExecuteOnce → `fyne.Do` deliver), `sendOrAbort`/`resetSendButton`/`setAbortButton`, `snapshotRequest`, `sessionTransport` (the cached per-session connection pool, #52), `panicResponse` (#110), `guard` (panic-recovery for callbacks, #48/#49), the `errorBanner` (#51) / `emptyState` (#58) toggles, and the `{{?Name}}` prompt-variable flow (#86): `requestTemplateStrings` gathers the request's template fields, `promptForVars` collects values in a Send-time dialog and re-enters `send` with them stashed in `promptSnap`. `recordHistory` logs each completed send to `Session.History()` (#65) — resolved method/URL for the list, the authored request snapshot for restore/resend. |
| [stream.go](stream.go) | The SSE streaming send (#74): the `Stream` button's `streamOrStop`/`streamSend` (snapshot on the UI goroutine → `httpclient.Stream` on a worker → coalesced repaints: events append to a worker-side buffer and at most one queued `fyne.Do` snapshots the newest text when it runs, so a burst of events costs one full re-parse, not one per event; the transcript stops accumulating at `displayBodyCap` — there is no tab cache behind a stream, so over-cap bytes would be invisible and unsaveable — with the truncation flagged on the status line), `formatSSEEvent`, and `setStreamStopButton`/`resetStreamButton`. A stream and a normal Send are mutually exclusive (each guards on the other's in-flight cancel func). |
| [websocket.go](websocket.go) | The WebSocket session (#72): `send` branches a `ws://`/`wss://` URL (`isWebSocketURL`) into `openWebSocket`, a modeless dialog that dials `internal/websocket` on a worker, streams received messages into a transcript and sends typed messages (`wsTranscriptLine`); closing the dialog closes the connection, or cancels the dial context when the handshake is still in flight (a late-arriving connection is closed at the publish gate instead of leaking). `currentRequestVars`/`currentRequestHeaders` resolve the open request's vars + headers into the URL and the upgrade handshake. |
...
| [execution.go](execution.go) | `sessionEnvBridge` (the `scripting.EnvBridge` adapter), `nilFinder` (the no-collection `chain.RequestFinder`), and `chainExecutor` — the single execution path that runs pre-script → `client.Do` → post-script for both chain steps and the leaf — plus `chainViewToScripting` and `formatTestResults` (renders `test()`/`expect()` outcomes #87 as `PASS`/`FAIL` console lines + summary). |
...
| [foldervars.go](foldervars.go) | `editFolderVariables` / `isFolderSelected` — opens `showVariablesEditor` on the selected folder's `Variables` (#81, a scope between environment and request) via `Session.FolderVariables` / `SetFolderVariables`. Reached from the sidebar's `folder-tree` button (`sbFolderVars`), gated to a folder selection by `refreshSidebarActions`. |
| [runcollection.go](runcollection.go) | `actionRunCollection` (#89) — runs requests off the UI goroutine via `runner.RunScope` and shows `showRunReport`, a per-request + aggregate pass/fail dialog. Scope follows the sidebar selection: a selected folder runs just that folder's subtree (activating its collection first), otherwise the whole active collection; the report header names the scope. Reached from the sidebar toolbar's `play` button ("Run collection (or selected folder)"); guarded by `runningCollection` against re-clicks. |
...
| [pathparams.go](pathparams.go) | The request editor's "Path" tab: `buildPathParamsTab`, `rebuildPathParamRows` (derives one row per `{name}` token in the URL via [internal/pathparam](../pathparam/), rebuilt on load + every URL edit; never mutates `PathParams` just by being shown), `buildPathParamRow`, `lookupPathParam`, `upsertPathParam` (write-back), and `filledPathParam` (mirrors `httpclient.Build` so the URL preview matches the wire). |
...
| [helpers.go](helpers.go) | Small UI utilities: `enableButton`, `tipButton` (named bundled SVG) / `tipButtonRes` (a pre-built icon resource, e.g. a Fyne `theme.*Icon()`), `thinHSplit`/`thinVSplit` (wrap the scope-free `thinSplit` widget), `flushColumn` (zero-spacing vertical layout with one flex row — the root assembly, replacing the old `rootTheme` zero-padding scope), `methodNames`, `bodyTypeNames`, `pruneEmptyKV`, `pruneEmptyChain`. |
| [filedialog.go](filedialog.go) | `showFileDialog` — the single seam every open/save/folder picker routes through: it enlarges the dialog to `fileDialogSize` (a pure, tested helper: ~85% of the window, floored so Fyne's large-icon default isn't cramped, clamped to the window) and shows it. |
| [thinsplit.go](thinsplit.go) | `thinSplit` — a drag-resizable two-pane split with a hard-coded 3px separator-coloured hairline divider (no grab handle), adapted from Fyne's `container.Split`. Exists so the main splits need no `container.NewThemeOverride` wrapping: every override call mints a Fyne theme scope, and Fyne re-parses fonts per scope × style and eagerly walks the wrapped subtree — the old `splitTheme`+`paneTheme` wrapping cost six scopes. |
...
| [help.go](help.go) | The Help menu behind the Help (question-mark) toolbar icon button (#61, #128): `helpMenuItems` (getting-started, shortcuts, **History** #65, web links: Website → idct.tech/helena, Report an issue → GitHub issues, "Buy me a coffee" with the BMC icon `assets.CoffeeIcon`, then About), `showHelpMenu`, `showGettingStarted`, `showAbout` (a custom dialog with Helena's photo — `assets.HelenaCat` — and the tribute note, plus version + repo link), `openURL`, and `SetVersion` (feeds About + the status-bar version segment). |
| [statusbar.go](statusbar.go) | The bottom bar (#-version-bar): `buildStatusBar` composes the leading transient `Status` line with a trailing group — the current version (`currentVersionText`) + an **opt-in** "Check for updates" link (`OnTapped`) + result label + a "Download" hyperlink (and, on Windows, a "Microsoft Store" link) — all regular-weight text so the bar shares one font + size. `checkForUpdates` runs the GitHub fetch off the UI goroutine (via the `fetchLatestRelease` test seam → [internal/updatecheck](../updatecheck/)) and marshals the verdict back with `fyne.Do` → `applyUpdateResult`. No automatic/background calls; the fetch happens only on a click. |
| [history.go](history.go) | The send-history viewer (#65): `showHistory` — a newest-first list of past sends (`historySummary` renders method / status / URL / relative `humanAgo` time) with Restore (`openScratchWith` the entry's request), Resend (open + `send`), and Clear (confirm → `Session.History().Clear()`). Reached from Help → History. Snapshots were secret-scrubbed at record time, so a restore keeps everything but the credential. |
...
| [auth.go](auth.go) | `buildAuthTab`, `loadAuthTab`, `refreshAuthVisibility`, `refreshAuthInheritLabel`, and the `ensureBasic`/`ensureBearer`/`ensureWSSE` (#79)/`ensureOAuth1` (#77)/`ensureAWSV4` (#76)/`ensureDigest` (#75)/`ensureNTLM` (#78)/`ensureAPIKey`/`ensureOAuth2` lazy allocators for the per-type sub-structs. |
...
| [helena_theme.go](helena_theme.go) | `helenaTheme` — Helena's custom `fyne.Theme`: a layered surface ramp, a single green brand accent (Primary/Focus/Selection/ForegroundOnPrimary), denser sizing, and the embedded Inter (UI) + JetBrains Mono (monospace) faces. Embeds `theme.DefaultTheme()` and delegates anything it doesn't override; `forced` pins the served variant for Light/Dark. `newHelenaTheme` builds it from a `model.Theme`. The scoped sub-themes all embed `delegatingTheme` — a stateless base that passes every `fyne.Theme` method through to the live app theme (`appTheme`) — and override only the few `Size`/`Color` cases that differ, so the pass-through boilerplate lives in one place (#56). `sidebarTheme` — a tree-scoped override (applied via `container.NewThemeOverride`) that only shrinks `SizeNameInlineIcon` + `SizeNamePadding` to make the collections tree's per-level indentation shallower and its icons denser. `toolbarTheme` is the same idea but enlarges `SizeNameInlineIcon` to 24 for the sidebar action toolbar. `tabsTheme` restores the 4pt active-tab indicator: Fyne 2.8 sizes the selected-tab underline from `SizeNameSeparatorThickness` (it used `SizeNamePadding` before), and `helenaTheme` pins that to 1 for hairline separators, so without the override the green accent pill collapses to a 1pt line. One `NewThemeOverride` around the request/response split covers both tab bars plus the Docs/Scripts sub-tabs nested inside; the export dialog wraps its own tabs. Scoped rather than global because `widget.NewSeparator` is structural in the toolbar, sidebar and status bar. (The old `splitTheme`/`paneTheme`/`rootTheme` scope overrides are gone — the thin split divider is now the scope-free [thinsplit.go](thinsplit.go) widget and the flush root joins the `flushColumn` layout in [helpers.go](helpers.go); only `sidebarTheme`, the two `toolbarTheme` wraps and the single `tabsTheme` wrap remain as theme scopes, cutting construction from 11 scopes to 4 and with it the per-scope font re-parsing.) `themedIcon(name)` wraps an embedded SVG in a `theme.ThemedResource` so Fyne recolours it to the foreground (Fyne only recolours high-importance button icons, so plain `currentColor` resources would render black on the medium/low toolbar buttons); results are memoized in `themedIcons` (UI-goroutine only) since construction requests ~21 icons with duplicates. |
| [tabs.go](tabs.go) | Editor tab-strip state on `MainUI`: the `openTab` / `tabResponse` types and `openOrActivate` / `activateTab` / `closeTab` / `closeAllTabs` / `reconcileTabs` / `newScratchTab` / `saveScratchTabAs` / `commitScratchTab`, the per-tab response helpers (`deliverResponse` / `applyResponse` / `clearResponsePanel`; `displayBodyCap` bounds what the shared viewer renders — 16 MiB, flagged on the status line — while the tab cache and Save response keep the full body), persistence (`persistTabs` / `restoreTabs`), the pool-based `rebuildTabBar` (which stamps each tab's dirty marker via `isTabDirty` — see [quit.go](quit.go)) and the per-edit `refreshActiveTabDirty` (updates just the active tab's marker so the asterisk tracks edits live), drag-reorder (`dragTab` / `otherTabCenters` / `applyDragTarget` / `dragEnd` + the pure `moveTabModel` / `dropIndex`), and the overflow menu (`tabMenuItems` / `showTabMenu`). Each `openTab` also carries a `cleanSnapshot` baseline read by the quit guard ([quit.go](quit.go)); `activateTab` captures it on first load. |
| [quit.go](quit.go) | The unsaved-edits quit guard (#139): `ConfirmQuit` (the single gate `cmd/helena`'s window-close intercept funnels through — quits immediately when clean or headless, else a `Discard & quit` / `Cancel` confirm) plus the dirty-tracking it reads. Request-field edits persist only on an explicit Save (every other mutation auto-saves), so the unsaved surface is per-request editor edits + scratch tabs, both anchored to a tab. Dirtiness is a value comparison, not a flag: `captureCleanSnapshot` records each tab's `cleanSnapshot` (the request marshaled via `marshalRequestState`, first load post-fold), `refreshCleanSnapshots` rebaselines a collection's tabs after a save (a save flushes the whole collection), and `isTabDirty` reports whether one tab diverges (a scratch tab with content, or a tree-backed tab whose marshaled live request differs from its snapshot). `unsavedTabCount`/`hasUnsavedEdits` sum it across tabs (flushing the debounced body editor first); the tab strip renders the same per-tab result as a trailing asterisk. |
| [saving.go](saving.go) | `ShowSaving` — the small buttonless "Saving…" spinner modal (`widget.Activity` + label) shown on the way out by `cmd/helena`'s close intercept, so a close that flushes state / tears down the GL context shows feedback instead of a frozen window. Never dismissed in-app; the process exits from under it after a short `savingSpinnerHold` (anti-flicker). No-op without a window. |
| [memtrim.go](memtrim.go) | `reclaimAfterLargeBody` — returns freed Go heap to the OS (off-UI, single-flighted `debug.FreeOSMemory`) after a large (≥ `memTrimThreshold`) response body is replaced or dropped: `applyResponse`/`clearResponsePanel`, `deliverResponse` to an inactive tab, `closeTab`/`closeAllTabs`/`reconcileTabs` (cached `tabResponse` bodies, summed via `cachedBodyLen`), and stream start. Go's scavenger is otherwise lazy, so RSS would ratchet across a session of big responses. `freeOSMemory` is a test seam. |
...
| [tabstrip.go](tabstrip.go) | `requestTab` — the per-tab custom widget (BaseWidget + `Tapped` + `Draggable` + a custom `requestTabRenderer`): a natural-width colored method chip, the name (with a trailing `*` when `setTab`'s `dirty` arg is set — the unsaved-edits marker), and a `circle-xmark` close button, all vertically centred (with left padding matching the close icon's right inset), over a rounded-top background. Active tabs fill with `ColorNameBackground` so they connect to the editor content and cover the under-strip separator line (Bruno-style); inactive tabs are transparent over the strip band. The band + line + leading/top spacing live in the strip `Stack` in [shell.go](shell.go) (`tabStripBg` + a bottom `widget.Separator`). |
...
| [responsewrap_test.go](responsewrap_test.go) | Sticky response soft-wrap: the viewer opens in the persisted mode (default off), a flip — via `SetWrap` **and** via the toolbar wrap button found in the live shell — reaches the session *and* the config file, and a response load never resets it. |
...
| [statusbar_test.go](statusbar_test.go) | Bottom-bar tests: `currentVersionText` mapping, the version segment + `SetVersion` refresh, and `applyUpdateResult` for each verdict (error / update / up-to-date / ahead / dev build). |
| [filedialog_test.go](filedialog_test.go) | `fileDialogSize` sizing: floor on unknown canvas, ~85% of a large window, clamp to a small window, and never exceeding the window. |
...
| `Request` | `*container.AppTabs` | Request editor tabs, in order: Body, Auth, Headers, Query, Path, Vars, Scripts, Chain, Assertions, Docs. ("Query" = the query-string params, two-way synced with the URL field — see `query.go`; "Path" = the URL's `{name}` path parameters, derived live from the URL — see `pathparams.go`; "Vars" = request-scoped variables, #82, see `requestvars.go`; "Assertions" = declarative checks, #88, see `assertions.go`.) |
...
| `Status` | `*widget.Label` | Transient status line — now the leading segment of the bottom bar built by `buildStatusBar` ([statusbar.go](statusbar.go)); still driven by `SetText` everywhere. |
| `statusVersion` | `*widget.Label` | Bottom-bar segment showing the running build's version (`currentVersionText`); refreshed by `SetVersion`. |
| `updateCheck` | `*widget.Hyperlink` | Opt-in "Check for updates" action (`OnTapped`, user-initiated). A Hyperlink, not a Button, so its label is regular weight like the rest of the bar (Fyne force-bolds button labels). |
| `updateStatus` | `*widget.Label` | Result text of the last update check ("Up to date", "Update available: vX", "Update check failed"…). |
| `updateChecking` | `bool` | Re-entrancy guard set while a check is in flight (replaces a button's disabled state; a Hyperlink has none). |
| `updateLink` | `*widget.Hyperlink` | "Download" link to the release page; shown only when a newer release exists. |
...
| `pathParamsRows` / `pathParamsHelp` | `*fyne.Container` / `*widget.Label` | The **Path** tab's rows (one per `{name}` token in the URL) and the help label shown when the URL has none. Rebuilt by `rebuildPathParamRows` on load + every URL edit — see `pathparams.go`. |
| `headersRows` | `*fyne.Container` | VBox of KV rows for headers. |
| `BodyType` | `*widget.Select` | Body type select (none / json / xml / text / graphql / form / multipart / file). |
...
| `pv` | `*prettyview.PrettyView` | The response **Body** viewer ([go-fyne-pretty-view](https://github.com/ideaconnect/go-fyne-pretty-view)): one virtualized widget rendering JSON/XML/HTML/raw with auto-detect, fold, syntax highlighting, search and soft-wrap. Fed via `pv.SetData` in `applyResponse`; subsumes the former Structured tree + Raw text viewer. Repainted on theme change via `pv.SetTheme(variantFor(...))`. Its **soft-wrap toggle is sticky**: constructed `WithWrap(wrapModeFor(sess.ResponseWrap()))` and wired through `SetOnWrapChanged` → `Session.SetResponseWrap`, so the mode survives a restart (a response load never resets it). |
