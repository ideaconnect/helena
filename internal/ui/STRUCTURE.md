...
| [stream.go](stream.go) | The SSE streaming send (#74): the `Stream` button's `streamOrStop`/`streamSend` (snapshot on the UI goroutine → `httpclient.Stream` on a worker → coalesced repaints: events append to a worker-side buffer and at most one queued `fyne.Do` snapshots the newest text when it runs, so a burst of events costs one full re-parse, not one per event), `formatSSEEvent`, and `setStreamStopButton`/`resetStreamButton`. A stream and a normal Send are mutually exclusive (each guards on the other's in-flight cancel func). |
