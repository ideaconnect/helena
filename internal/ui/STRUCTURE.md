...
| [memtrim.go](memtrim.go) | `reclaimAfterLargeBody` — returns freed Go heap to the OS (off-UI, single-flighted `debug.FreeOSMemory`) after a large (≥ `memTrimThreshold`) response body is replaced/cleared in `applyResponse`/`clearResponsePanel`, so RSS doesn't ratchet across a session of big responses. Go's scavenger is otherwise lazy. `freeOSMemory` is a test seam. |
