...
| [runcollection.go](runcollection.go) | `actionRunCollection` (#89) — runs the active collection off the UI goroutine via `runner.Run` and shows `showRunReport`, a per-request + aggregate pass/fail dialog. Reached from the sidebar toolbar's `play` button; guarded by `runningCollection` against re-clicks. |
