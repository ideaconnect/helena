...
| [auth.go](auth.go) | `buildAuthTab`, `loadAuthTab`, `refreshAuthVisibility`, `refreshAuthInheritLabel`, and the `ensureBasic`/`ensureBearer`/`ensureWSSE` (#79)/`ensureOAuth1` (#77)/`ensureAWSV4` (#76)/`ensureAPIKey`/`ensureOAuth2` lazy allocators for the per-type sub-structs. |
