...
| [shell.go](shell.go) | `MainUI` struct, `NewMainUI`, the main layout, send/save/loadRequest, params/headers row machinery, environment + settings dialogs, body validate/format. The request body uses an **editable** `go-fyne-pretty-view` widget (`BodyContent`); `formatForBodyType` maps the body type to its syntax format and `syncBodyFromEditor` pulls the debounced editor's live `Source()` before Save/Send/Validate/Format. Also defines `sessionEnvBridge` (the `scripting.EnvBridge` adapter), `sessionRequestFinder` (the `chain.RequestFinder` adapter), and `chainExecutor` — the single execution path that runs pre-script → `client.Do` → post-script for both chain steps and the leaf. `loadErrorReport`/`SurfaceLoadErrors` format and show (via `SetOnStarted` in `cmd/helena`) a non-transient diagnostic for collections the session failed to load (#108). `guard(label, fn)` wraps dialog/action callbacks (import, save settings, save environment) so a panic in an event handler is recovered, logged, and surfaced as an error dialog instead of crashing the process (#48). `errorBanner`/`showErrorBanner`/`hideErrorBanner` provide the persistent, dismissible failure surface above the response tabs (#51 — see WORKFLOW "Error-surfacing contract"). |
| [query.go](query.go) | Pure helpers for the two-way **Query**↔URL sync: `splitURLQuery`, `parseQueryParams`, `buildQueryString`, `displayURL`, `mergeQueryFromURL` (keeps disabled rows), and `encodeQueryComponent` (percent-encodes but leaves `{{vars}}` intact). The UI glue (`applyURLEdit`, `syncURLFieldFromParams`) lives in [shell.go](shell.go), where `kvRow` (the row's widget handles) + `syncParamsRowsInPlace` let a value-only URL edit update existing Query rows in place instead of a full `RemoveAll`+rebuild per keystroke (#53). `currentRequest.Params` stays the send-path source of truth, so the backend is unchanged. |
