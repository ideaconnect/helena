...
| [statusbar.go](statusbar.go) | The bottom bar (#-version-bar): `buildStatusBar` composes the leading transient `Status` line with a trailing group — the current version (`currentVersionText`) + an **opt-in** "Check for updates" button + result label + a "Download" hyperlink (and, on Windows, a "Microsoft Store" link). `checkForUpdates` runs the GitHub fetch off the UI goroutine (via the `fetchLatestRelease` test seam → [internal/updatecheck](../updatecheck/)) and marshals the verdict back with `fyne.Do` → `applyUpdateResult`. No automatic/background calls; the fetch happens only on a click. |
...
| `updateCheckBtn` | `*widget.Button` | Opt-in "Check for updates" button (user-initiated; disabled while a check is in flight). |
