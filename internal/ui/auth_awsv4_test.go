...
func TestAuthTabLoadsAWSV4(t *testing.T) {
	...
	if got := m.authEd.typeSel.Selected; got != "AWS Signature v4" {
		t.Errorf("authType.Selected = %q, want AWS Signature v4", got)
	}
	if got := m.authEd.awsAccessKey.Text; got != "AKID" {
		t.Errorf("access key = %q, want AKID", got)
	}
	if got := m.authEd.awsRegion.Text; got != "eu-west-1" {
...
func TestAuthTabAWSV4WriteBack(t *testing.T) {
	...
	m.authEd.awsAccessKey.OnChanged("AKID")
	m.authEd.awsSecretKey.OnChanged("secret")
	m.authEd.awsRegion.OnChanged("us-west-2")
	m.authEd.awsService.OnChanged("s3")
	m.authEd.awsSessionToken.OnChanged("tok")
