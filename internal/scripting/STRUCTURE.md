...
| [bindings.go](bindings.go) | All internal binding helpers: `bindHelena`, `bindConsole`, `stringify`, `runGuarded` / `runWithTimeout`, `requestToObject` / `writeBackGuarded` / `writeBackRequest` / `mergeKVFromObject`, `responseToObject`, `tryParseJSON`, `parseSendSpec` (#92). |
...
| `writeBackGuarded` | Runs `writeBackRequest` under `runGuarded` so a hostile request-object getter or value `toString` (attacker-controllable JS) is interrupted instead of hanging the Send worker after the script's own guard is gone. Reads into a copy, committing to `*model.Request` only on success, so an abandoned read-back can't race the outgoing request. |
| `writeBackRequest` | Reads the (possibly mutated) JS request object back into `*model.Request`. Scalars are direct writes; headers and params go through `mergeKVFromObject`. Called only via `writeBackGuarded`. |
