...
| `ocAuthBasic` / `ocAuthBearer` / `ocAuthAPIKey` / `ocAuthOAuth2` / `ocAuthWSSE` / `ocAuthOAuth1` | the credential sub-blocks under `auth.<type>` | `model.BasicAuth` / `BearerAuth` / `APIKeyAuth` / `OAuth2Auth` / `WSSEAuth` (#79) / `OAuth1Auth` (#77) |
