...
| [secrets.go](secrets.go) | Secret externalization (#42): split secret fields out of the collection YAML into a config-dir store on Save, merge back on Load. Covers request/folder/collection auth, environment variables, and Secret-flagged collection variables (#80). Also exports `ScrubRequestSecrets` — a deep copy of a request with the same secret fields (auth + Secret-flagged request vars) blanked, used by the request history (#65) so a snapshot on disk carries no cleartext credential; it reuses `authSecrets` / `variableSecrets`, so the field list can't drift from the externalizer. |
