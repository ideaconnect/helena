...
| [auth.go](auth.go) | `Auth` plus its sub-structs (`BasicAuth`, `BearerAuth`, `APIKeyAuth`, `OAuth2Auth`, `WSSEAuth` #79, `OAuth1Auth` #77) and the related enums (`AuthType`, `APIKeyPlacement`, `OAuth2Grant`). Applied by [internal/auth](../auth/). |
...
- `Basic` / `Bearer` / `APIKey` / `OAuth2` / `WSSE` / `OAuth1` — pointer fields; exactly one is non-nil for a concrete auth.
...
Typed string enums. `AuthType` covers `none`, `inherit`, `basic`, `bearer`, `apikey`, `oauth2`, `wsse`, `oauth1`. `APIKeyPlacement` is `header` or `query`. `OAuth2Grant` is `client_credentials` or `authorization_code`.
