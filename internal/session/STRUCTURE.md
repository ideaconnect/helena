...
| File | Responsibility |
| --- | --- |
| [session.go](session.go) | `Session` type, constructor `New`, workspace switching, active collection / environment (incl. `RemoveCollection` — an in-place delete that preserves the OTHER collections' unsaved in-memory edits and reindexes the per-collection env map, instead of reloading the workspace from disk; and `MoveCollection` — reorders the top-level collection list with final-index semantics, keeping `cols`/`dirs`/workspace in lockstep, the active pointer on its collection, and remapping the index-keyed `activeEnv` so each collection's active environment follows it to its new slot), UI state persistence (open request, open tabs, window size, response soft-wrap via `SetResponseWrap` / `ResponseWrap` — like `SetOpenTabs` it skips the config write when the value is unchanged, since the viewer's toggle fires on every flip, settings), `Resolver()` / `ResolverForNode()` / `ResolverForRequest()` (folder-scoped #81 + request-scoped #82 variables) and the `enabledVars` helper, folder variables (#81: `SnapshotAncestorVars` / `FolderVariables` / `SetFolderVariables`), folder / collection settings persistence (`FolderAuth`, `UpdateFolder`, `UpdateCollection` — one write for variables + auth, the sidebar settings dialogs' save path) and `InheritedAuth` (the ancestors-only auth walk behind the Inherit preview; `EffectiveAuth` is the Send-time value), global variables (#83: `GlobalVariables` / `SetGlobalVariables` / `SnapshotGlobalVars`, the lowest scope, persisted in config), env overlay (`SetEnvOverlay` / `EnvOverlay` / `ClearEnvOverlay` / `SnapshotEnvOverlay` / `SnapshotActiveEnvVars`), request location for the UI tab strip (`LocateRequest`, `ContainerPaths` / `ContainerRef`, `CollectionDir`), open-tab persistence (`SetOpenTabs` / `OpenTabs`; both it and `SetActiveCollection` skip the config write when the value is unchanged — they fire on every tab switch), `SaveActiveCollection` / `saveCollection`, `History()` (the #65 send-history store, built from `historyPath(cfgPath)`), and `FindRequestByPath` used by [internal/chain](../chain/). |
...
| [items.go](items.go) | Tree mutation: `AddRequest`, `AddRequestValue`, `AddFolder`, `RenameItem`, `DeleteItem`, `DuplicateItem`, `MoveNode`, plus one-step delete undo (#68): `DeleteItem` snapshots the removed node into `deletedNode` — capturing the parent container by stable folder `model.ID` (`parentFolderID`), not just its positional path — and `RestoreLastDeleted` / `CanUndoDelete` / `LastDeletedName` re-insert it at its original (clamped) position, re-resolving the parent by that ID so an intervening index-shifting op can't retarget the restore into the wrong container. A failed delete-save (which `reload()`s the item back into memory) clears `lastDeleted`, so a later undo can't re-insert a duplicate. `cloneRequestKeepID` / `cloneFolderKeepID` deep-copy while preserving IDs (unlike the duplication clones), so chain refs survive a delete→restore. Each mutator persists. `MoveNode` relocates a folder/request within the same collection (drag-and-drop): it removes the item, re-resolves the destination container by a stable folder `model.ID` (the removal shifts positional node IDs), inserts, cascades chain refs like a rename, and saves. Helpers: `folderIDForContainer`, `containerByFolderID`, `nodeIDOfModel`, `clampIndex`, and `FolderNodeID` (name-path → folder node id, the inverse of `nodeNamePath`, used by `helena run --folder` #89). |
...
| [session_containerauth_test.go](session_containerauth_test.go) | `InheritedAuth` (ignores the node's own auth; folder / root request / collection root / unknown id), `FolderAuth` (own value; false for non-folders), `UpdateFolder` / `UpdateCollection` (persist auth + variables in one write, descendants inherit after reload, bad ids / indices error without calling `mutate`). |
...
| `history` | `*history.Store` | Bounded, restart-persistent send history (#65), returned by `History()`. Constructed in `New` from `historyPath(cfgPath)` (a blank config path → in-memory only, e.g. a headless run). Snapshots are secret-scrubbed before persisting to `history.yml`. |
