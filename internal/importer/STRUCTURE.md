...
| [openapi.go](openapi.go) | OpenAPI 3 / Swagger 2 parsing: `FromOpenAPI`, YAML→JSON normalization, OAS3-to-collection conversion. Parameters route by `in`: `query`→`Params`, `header`→`Headers`, `path`→`PathParams` (kept enabled; the `{name}` token stays in the URL so the Path tab fills it). The operation loop iterates `PathItem.Operations()`, so it picks up OpenAPI 3.2's `query` field and `additionalOperations` map as well as the classic verbs — methods outside `model.Methods` are imported deliberately rather than dropped (`TestFromOpenAPI32CustomMethods` pins it). |
