...
1. **Source declaration.** [assets.go](assets.go) carries the
   `//go:embed app_icon_window.png` directive immediately above
...
2. **Compile time.** When `go build` processes the package, the toolchain
   reads `app_icon_window.png` (the 256×256 window icon) from the package directory and bakes its bytes
...
dependency. To replace the icon, overwrite `assets/app_icon.png` (the
full-res art), regenerate the embedded downscale with
`convert app_icon.png -resize 256x256 -strip app_icon_window.png`, and
