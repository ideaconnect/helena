...
- [assets.go](assets.go) — package declaration, the three `//go:embed`
  ...
  accessors.
- `app_icon.png` — the PNG bytes embedded into `AppIcon` (the Helena cat
  mascot, used as the app/taskbar icon and the in-window title-bar icon).
...
No regeneration step exists — to replace the icon swap `app_icon.png` in place;
...
- `AppIcon []byte` — the PNG bytes of the application/window icon, loaded via
