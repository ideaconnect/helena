...
| [run.go](run.go) | The `helena run <collection-dir>` headless subcommand (#90), dispatched from `main` before the GUI starts: `runCommand` parses `--env` / `--format` / `--folder` (two-phase parse so flags work before or after the dir), opens the collection into an ephemeral session, resolves `--folder` to a node id via `session.FolderNodeID`, calls `runner.RunScope`, and `writeReport`s the result in the chosen format. `printReport` renders the human-readable text summary; `writeReport` selects text / `runner.Report.JSON()` / `runner.Report.JUnit()`. Exit codes: 0 clean, 1 any request errored / check failed, 2 usage/setup error (incl. an unknown `--folder`). |
| [run_test.go](run_test.go) | Tests for `printReport` (ok/FAIL/summary lines), `writeReport` (json parses, junit parses, text summary), the two-phase flag parse (`--format` after the dir is honored), unknown-`--format` rejection, and `--folder` scoping (existing folder runs, unknown folder → exit 2). |
